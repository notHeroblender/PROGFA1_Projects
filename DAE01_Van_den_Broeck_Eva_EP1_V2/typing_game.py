...
#EXAM
dae_btn_img:ProgfaImage
dae_btn_x, dae_btn_y = 0,0
dae_btn_width, dae_btn_height = 100,40
...
def init_start():
    ...
    global bg_img, btn_img, diff_btn_img, btn_offset, btn_width, btn_height,\
        ...
        current_state, dae_btn_img, dae_btn_x, dae_btn_y
    ...
    #EXAM
    dae_btn_img = engine.load_image("Resources/button_image.png")
    dae_btn_img.resize(dae_btn_width,dae_btn_height)
    dae_btn_x = 20
    dae_btn_y = engine.height-dae_btn_img.height-20
...
def render():
    ...
    if current_state == GameState.END:
        draw_end()
    if current_state == GameState.CHEAT:
        draw_cheat()
...
def draw_cheat():
    """
    draws the first 10 words of the current word list on screen and again next to it, but with "DAE" added
    """
    cheat_words = []
    cheat_words.extend(current_words)
    cheat_words.insert(0,"DAE")

    engine.background_color = 1,1,1
    engine.color = 0,0,0
    engine.set_font_size(20)
    for i in range(10):
        engine.draw_text(current_words[i],15, 15+25*i)
        engine.draw_text(cheat_words[i], engine.width/2, 15+25*i)
    pass
...
def draw_start():
    ...
    #EXAM
    dae_btn_img.draw(dae_btn_x,dae_btn_y)
...
def mouse_pressed_event(mouse_x: int, mouse_y: int, mouse_button: MouseButton):
    ...
    if current_state == GameState.START:
        if (easy_btn_x < mouse_x < easy_btn_x+diff_btn_width) and (diff_btn_y<mouse_y<diff_btn_y+btn_height):
            ...
            quit()
        #EXAM
        if dae_btn_clicked(mouse_x,mouse_y):
            current_state = GameState.CHEAT
    ...
    if current_state == GameState.END:
        ...
        if (again_btn_x < mouse_x < again_btn_x+btn_img.width) and (again_btn_y < mouse_y < again_btn_y+btn_img.height):
            ...
            reset_game()
            current_state = GameState.PLAY
    if current_state == GameState.CHEAT and mouse_button == mouse_button.LEFT :
        current_state = GameState.END

    pass

#EXAM
def dae_btn_clicked(x,y)->bool:
    """
    checks if the player clicked inside the dae button
    :param x: mouse x pos
    :param y: mouse y pos
    :return: true or false
    """
    if (dae_btn_x < x < dae_btn_x + dae_btn_width) and (dae_btn_y < y < dae_btn_y + dae_btn_height):
        print("True")
        return True
    else:
        print("False")
